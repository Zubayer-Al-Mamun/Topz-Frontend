...
export default function Buy() {
    ...
    // সঠিক ডিসকাউন্ট প্রাইস হিসাব করা (database-এর discountsPrice: 0 ওভাররাইড করার জন্য)
    let discountsPrice = Math.ceil(
        (product?.pricing || 0) * (1 - (product?.discountsPercentage || 0) / 100)
    );

    // codCharge নিরাপদ করার জন্য Number() এবং fallback (0) ব্যবহার করা হয়েছে যাতে NaN না আসে
    let codCharge = product?.codChargeStatus
        ? Number(product.codCharge) || 0
        : Number(import.meta.env.VITE_COD_CHARGE) || 0;
                ...
                items: [
                    {
                        productId: product._id,
                        imageUrl: product.images,
                        // product.discountsPrice এর জায়গায় ক্যালকুলেট করা discountsPrice দেওয়া হলো
                        pricing: discountsPrice,
                        qty: 1,
                        selectedSize: "base",
                    },
                ],
    ...
    // totalPrice হিসাব নিরাপদ করার জন্য Number() এবং fallback যোগ করা হয়েছে
    const totalPrice =
        order.items.reduce(
            (sum, item) => sum + (Number(item.pricing) || discountsPrice) * (Number(item.qty) || 1),
            0,
        ) + codCharge;
                            ...
                            {saveAddresses.phone != null && !newAddress && (
                                <div className="">
                                    ...
                                    <div className=" mb-1 border-2 border-green-400 bg-green-200 p-2 rounded-md flex justify-between">
                                        ...
                                        <div
                                            className="flex items-center justify-center h-7 w-7 cursor-pointer"
                                            onClick={() => {
                                                setOrder((prev) => ({
                                                    ...prev,
                                                    customerDetails: {
